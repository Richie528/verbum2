...
// Ensure redirect from test page
if (storage.read("from-test") !== "true") {
    // window.location.href = window.location.href.substring(0, window.location.href.length - 19);
    window.location.href = window.location.href.substring(0, window.location.href.length - 19) + "/index.html";
}
storage.write("from-test", false);
...
let stats = {
    ...
    init() {
        this.wordsTested = parseInt(storage.read("words-tested"));
        this.wordsCorrect = parseInt(storage.read("words-correct"));
        this.cycles = parseInt(storage.read("cycles"));
        this.wordResults = JSON.parse(storage.read("word-results"));
        this.stageResults = JSON.parse(storage.read("stage-results"));
    }
};
stats.init();

let resultsOptions = {
    category: parseInt(storage.read("results-options-category", 0)),
    sort: parseInt(storage.read("results-options-sort", 0)),
    categoryButtons: document.querySelector(".results-options-category").children,
    sortButtons: document.querySelector(".results-options-sort").children,
    
    save() {
        storage.write("results-options-category", this.category);
        storage.write("results-options-sort", this.sort);
    },
    display() {
        for (let i = 0; i < 2; i++) this.categoryButtons[i].classList.remove("results-options-button-selected");
        for (let i = 0; i < 3; i++) this.sortButtons[i].classList.remove("results-options-button-selected");
        this.categoryButtons[this.category].classList.add("results-options-button-selected");
        this.sortButtons[this.sort].classList.add("results-options-button-selected");
    },
    init() {
        if (Number.isNaN(this.category)) this.category = 0;
        if (Number.isNaN(this.sort)) this.sort = 0;
        for (let i = 0; i < 2; i++) {
            this.categoryButtons[i].onclick = function() {
                resultsOptions.category = i;
                resultsOptions.display();
                results.display();
                resultsOptions.save();
            };
        }
        for (let i = 0; i < 3; i++) {
            this.sortButtons[i].onclick = function() {
                resultsOptions.sort = i;
                resultsOptions.display();
                results.display();
                resultsOptions.save();
            };
...
let mainResults = {
    text: document.querySelector(".main-results-text"),
    bar: document.querySelector(".main-results-bar-inner"),

    init() {
        this.text.textContent = `${stats.wordsCorrect}/${stats.wordsTested}`;
        this.bar.style.width = `${stats.wordsCorrect / stats.wordsTested * 100}%`;
    }
};
mainResults.init();
...
let results = {
    ...
    display() {
        if (resultsOptions.category === 0) this.list = Object.entries({...stats.wordResults});
        if (resultsOptions.category === 1) this.list = Object.entries({...stats.stageResults});

        if (resultsOptions.sort === 0) {
            this.list.sort(function(a, b) {
                if (resultsOptions.category === 0) return (word[a[0]][0] < word[b[0]][0]) ? -1 : 1;
                if (resultsOptions.category === 1) return (a[0] < b[0]) ? -1 : 1;
            });
        }
        if (resultsOptions.sort === 1) {
            this.list.sort(function(a, b) {
                if (a[1][0] === b[1][0]) {
                    if (a[1][1] === b[1][1]) {
                        if (resultsOptions.category === 0) return (word[a[0]][0] < word[b[0]][0]) ? -1 : 1;
        ...
        if (resultsOptions.sort === 2) {
            this.list.sort(function(a, b) {
                ...
                return (a[1][1] - a[1][0] > b[1][1] - b[1][0]) ? -1 : 1;
            });
        }

        this.resultsList.innerHTML = ``;
        for (let [id, stats] of this.list) {
            if (stats[1] === 0) continue;
            let resultListItem = document.createElement("div");
            resultListItem.classList.add("results-list-item");
            resultListItem.innerHTML = 
            `
                <div style="display:flex">
                    <div class="results-li-stats">${stats[0]}/${stats[1]}</div>
                    <div class="results-li-name">${resultsOptions.category === 0 ? word[id][0] :  "Stage " + (parseInt(id) + 1)}</div>
                    <div class="results-li-translation">${resultsOptions.category === 0 ? word[id][3] : ""}</div>
                </div>
                <div class="results-li-bar">
                    <div class="results-li-bar-inner" style="width: ${stats[0] / stats[1] * 100}%"></div>
                </div>
            `;
