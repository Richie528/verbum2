...
let utility = {
    ...
    check(input, translations) {
        input = input.split(",");
        for (let i = 0; i < input.length; i++) input[i] = this.clean(input[i]);
        for (let i = 0; i < input.length; i++) {
            let isCorrect = false;
            for (let j = 0; j < translations.length; j++) {
                if (input[i] === this.clean(translations[j])) isCorrect = true;
            }
...
// Ensure redirect from home page
if (storage.read("from-home") !== "true") {
    // window.location.href = window.location.href.substring(0, window.location.href.length - 16);
    window.location.href = window.location.href.substring(0, window.location.href.length - 16) + "/index.html";
}
storage.write("from-home", false);

let stages = {
    selected: storage.read("stages-selected", "00000000000000000000000000000000000000000"),
    buttons: document.querySelectorAll(".stage-button"),

    display() {
        for (let i = 0; i < 41; i++) {
            if (this.selected[i] === "1") this.buttons[i].classList.add("stage-button-selected");
            else this.buttons[i].classList.remove("stage-button-selected");
        }
    },
    init() {
        this.display();
...
let testOptions = {
    value: parseInt(storage.read("test-options-value", 0)),
    option: parseInt(storage.read("test-options-option", 0)),
    text: document.querySelector(".test-options"),
    
    display() {
        this.text.textContent = "";
        let testOptionsText = ["Cycle", "Word", "Minute", "Second", "Infinite"];
        this.text.textContent += testOptionsText[this.option];
        this.text.textContent += " - ";
...
let wordlist = {
    ...
    recycleList() {
        utility.shuffleArray(this.list);
        this.index = 0;
    },
    wordInfo(id) {
        return [word[id][2], word[id][1], word[id][0], word[id][3]];
    },
    getNextWord() {
        let word = this.list[this.index];
        this.index += 1;
        if (this.index === this.list.length) this.recycleList();
        return word;
    },
    init() {
        for (let i = 0; i < 41; i++) {
            if (stages.selected[i] === "1") {
                for (let w of stage[i]) {
                    this.list.push([w, i]);
                }
            }
        }
...
let stats = {
    ...
    calcCycles() {
        this.cycles = Math.floor(this.wordsTested / wordlist.list.length);
    },
    init() {
        for (let word of wordlist.list) {
            this.wordResults[word[0]] = [0, 0];
        }
        for (let i = 0; i < stages.selected.length; i++) {
            if (stages.selected[i] === "1") {
                this.stageResults[i] = [0, 0];
            }
        }
    }
};
...
let words = {
    animationStart: ["current", "next", "gone"],
    animationEnd: ["gone", "current", "next"],
    main: document.querySelector(".test"),
    words: [],

    transition() {
        test.inputCooldown = false;
        // Create new word
        let newWord = wordlist.getNextWord();
        let newWordInfo = wordlist.wordInfo(newWord[0]);
        let newWordElement = document.createElement("div");
        newWordElement.innerHTML = 
        `
            <div class="part-of-speech">${newWordInfo[0]}</div>
            <div class="full-name">${newWordInfo[1]}</div>
            <div class="short-name">${newWordInfo[2]}</div>
            <input class="test-input" maxlength="100">
        `;
        newWordElement.setAttribute("data-word", JSON.stringify(newWord));
        newWordElement.classList.add("word", "gone");
        this.main.appendChild(newWordElement);
        newWordElement.offsetHeight;

        // Shift
        this.words = this.main.querySelectorAll(".word");
        for (let i = 0; i < 3; i++) {
            this.words[i].classList.add(this.animationEnd[i]);
            this.words[i].classList.remove(this.animationStart[i]);
        }
        test.next();

        wordToRemove = this.main.querySelector(".gone");
        wordToRemove.classList.remove("word");
        // Remove old word
        setTimeout(function() {
            words.main.removeChild(wordToRemove);
            test.inputCooldown = true;
    ...
    init() {
        ...
        for (let i = 0; i < 2; i++) {
            ...
            `
                ...
                <input class="test-input" maxlength="100">
            `;
            this.words[i].setAttribute("data-word", JSON.stringify(newWord));
        }
    }
};
words.init();
...
let test = {
    ...
    chances: 2,
        ...
        stats.calcCycles();
        if (testOptions.option === 0) if (stats.cycles >= testOptions.value) endTest();
        if (testOptions.option === 1) if (stats.wordsTested >= testOptions.value) endTest();
    },
    next() {
        document.querySelector(".next").querySelector(".test-input").disabled = true;
        this.input = document.querySelector(".current").querySelector(".test-input");
        this.input.disabled = false; this.input.focus();
        this.input.addEventListener("keypress", function(e) {
            if (e.key === "Enter") {
                if (!test.inputCooldown) return;
                let inputWord = JSON.parse(test.input.parentElement.getAttribute("data-word"));
                if (utility.check(test.input.value, wordlist.wordInfo(inputWord[0])[3]) && test.input.style.color != "var(--red)") {
                    // correct
                    test.correct(inputWord);
                } else {
                    // incorrect
                    test.wrong(inputWord);
                }
            }
        });
        this.chances = 2;
    ...
    correct(inputWord) {
        ...
        stats.wordResults[inputWord[0]][0] += 1;
        stats.wordResults[inputWord[0]][1] += 1;
        stats.stageResults[inputWord[1]][0] += 1;
        stats.stageResults[inputWord[1]][1] += 1;
    ...
    wrong(inputWord) {
        this.chances -= 1;
        if (this.chances > 0) {
            this.input.style.animation = "shake 0.2s";
            setTimeout(function() {
                test.input.style.animation = "none";
                test.input.value = "";
            }, 200);
            ...
            this.input.style.color = "var(--red)";
            this.input.value = wordlist.wordInfo(inputWord[0])[3];
        } else {
            this.input.disabled = true;
            words.transition();
        }
    ...
    init() {
        this.next();
        if (testOptions.option === 2 || testOptions.option === 3) {
            setTimeout(function() {endTest();}, testOptions.option === 2 ? testOptions.value * 1000 : testOptions.value * 60000);
        }
    }
};
test.init();
...
function endTest() {
    storage.write("from-test", true);
    storage.write("words-tested", stats.wordsTested);
    storage.write("words-correct", stats.wordsCorrect);
    storage.write("cycles", stats.cycles);
    storage.write("word-results", JSON.stringify(stats.wordResults));
    storage.write("stage-results", JSON.stringify(stats.stageResults));

    window.location.href = window.location.href.substring(0, window.location.href.length - 15) + "results/index.html";
}

document.querySelector(".end-test-button").onclick = endTest;