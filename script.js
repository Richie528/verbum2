...
let stages = {
    ...
    save() {
        storage.write("stages-selected", this.selected);
    },
    display() {
        let wordCount = 0;
        for (let i = 0; i < 41; i++) {
            if (this.selected[i] === "1") {
                this.buttons[i].classList.add("stage-button-selected");
                wordCount += stage[i].length;
            }
            else this.buttons[i].classList.remove("stage-button-selected");
        }
        document.getElementById("num-words-selected").textContent = wordCount;
    },
    init() {
        for (let i = 0; i < 41; i++) {
            this.buttons[i].onclick = function(e) {
                if (e.shiftKey) stages.selected = ((stages.selected[i] === "0") ? "1" : "0").repeat(i + 1) + stages.selected.substring(i + 1);
                else stages.selected = stages.selected.substring(0, i) + ((stages.selected[i] === "0") ? "1" : "0") + stages.selected.substring(i + 1);
                stages.display();
                stages.save();
            };
        }
...
let testOptions = {
    ...
    buttons: document.querySelectorAll(".test-options-button"),
    
    save() {
        storage.write("test-options-value", this.value);
        storage.write("test-options-option", this.option);
    },
    display() {
        for (let i = 0; i < 5; i++) this.buttons[i].classList.remove("test-options-button-selected");
        this.buttons[this.option].classList.add("test-options-button-selected");
        document.getElementById("test-options-value").value = this.value;
    },
    init() {
        if (Number.isNaN(this.option)) this.option = 0;
        if (Number.isNaN(this.value)) this.value = 0;
        document.getElementById("test-options-value").onclick = function() {
            document.getElementById("test-options-value").select();
        }
        document.getElementById("test-options-value").addEventListener("keyup", function() {
            testOptions.value = parseInt(document.getElementById("test-options-value").value);
            testOptions.save();
        });
        for (let i = 0; i < 5; i++) {
            this.buttons[i].onclick = function() {
                testOptions.option = i;
                testOptions.display();
                testOptions.save();
            };
        }
...
function startTest() { 
    storage.write("from-home", true);

    if (stages.selected === "00000000000000000000000000000000000000000") {
        document.querySelector(".select-stages-prompt").style.animation = "shake 0.2s";
        setTimeout(function() {document.querySelector(".select-stages-prompt").style.animation = "none";}, 200);
        return;
    }
    if (Number.isNaN(testOptions.value)) {
        document.getElementById("test-options-value").style.animation = "shake 0.2s";
        setTimeout(function() {document.getElementById("test-options-value").style.animation = "none";}, 200);
        return;
    }
...
document.querySelector(".start-button").onclick = startTest;
